...
{
    /// <summary>
    /// Descripción breve de jqGridIngresos
    /// </summary>
    public class jqGridIngresos : IHttpHandler
    {

        public void ProcessRequest(HttpContext context)
        {
            string reg = "", com = "", sol = "", tipo = "", mes = "", ano = "", rutp = "", rute = "", orden = "", fi = "", ff = "", est = "", di = "", nc = "", estep = "", usr = "", idprof = "", idprofev = "";
            ...
            ano = "";
            if (context.Request["a"] != "" && context.Request["a"] != "0" && context.Request["a"] != "null" && context.Request["a"] != null)
            {
                ano = " and Year(S.fecha_solicitud) ='" + context.Request["a"] + "'";
            }

            tipo = "";
            if (context.Request["t"] != "" && context.Request["t"] != "0" && context.Request["t"] != "null" && context.Request["t"] != null)
            {
                tipo = " and s.ID_TIPO_SOLICITUD IN (" + context.Request["t"] + ")";
            }

            idprof = "";
            if (context.Request["ip"] != "" && context.Request["ip"] != "0" && context.Request["ip"] != "null" && context.Request["ip"] != null)
            {
                //idprof = " and s.ID_PROFESIONAL IN (" + context.Request["ip"] + ")";
                idprof = @" AND EXISTS  (Select 1 from ACTIVIDADES fa 
                inner join PROFESIONALES fp on fp.ID_PROFESIONAL=fa.ID_PROFESIONAL 
                INNER JOIN TIPO_ACTIVIDAD fTP ON fTP.ID_TIPO_ACTIVIDAD=fA.ID_TIPO_ACTIVIDAD 
                where fa.ID_SOLICITUD= s.ID_SOLICITUD and fTP.ID_TIPO_SOLICITUD NOT IN (1,2,3,4) and fp.ID_PROFESIONAL in (" + context.Request["ip"] + ")) ";
            }

            idprofev = "";
            if (context.Request["ipev"] != "" && context.Request["ipev"] != "0" && context.Request["ipev"] != "null" && context.Request["ipev"] != null)
            {
                //idprofev = " and s.ID_PROFESIONAL_EVA IN (" + context.Request["ipev"] + ")";
                idprofev = @" AND EXISTS  (Select 1 from ACTIVIDADES fa 
                inner join PROFESIONALES fp on fp.ID_PROFESIONAL=fa.ID_PROFESIONAL 
                INNER JOIN TIPO_ACTIVIDAD fTP ON fTP.ID_TIPO_ACTIVIDAD=fA.ID_TIPO_ACTIVIDAD 
                where fa.ID_SOLICITUD= s.ID_SOLICITUD and fTP.ID_TIPO_SOLICITUD IN (1,2,3,4) and fp.ID_PROFESIONAL in (" + context.Request["ipev"] + ")) ";
            }

            rutp = "";
            if (context.Request["rp"] != "")
            {
                rutp = " and (P.RUT like '%" + context.Request["rp"].Replace(".", "") + "%' OR P.NOMBRE like '%" + context.Request["rp"].Replace(".", "") + "%')";
            }

            rute = "";
            if (context.Request["re"] != "")
            {
                rute = " and (E.RUT like '%" + context.Request["re"].Replace(".", "") + "%' OR E.NOMBRE like '%" + context.Request["re"].Replace(".", "") + "%')";
            }
            ...
            DataSet dsQuery = fn.dsSql("SELECT S.ID_SOLICITUD,F_SOLICITUD=CONVERT(VARCHAR(10),S.FECHA_SOLICITUD, 105),TIPO=TP.NOMBRE,SOL=dbo.MayMinTexto(U.NOMBRE),RUT_E=E.RUT," +
                                       "NOM_E=dbo.MayMinTexto(E.NOMBRE),TRAB_RUT=P.RUT,TRAB_NOM=dbo.MayMinTexto(P.NOMBRE),S.ORDEN_SINIESTRO,S.ESTADO,A.AGENCIA,ddf=ISNULL(dbo.DIAS_TRABAJO_EPT(s.ID_SOLICITUD, 1), '')," +
                                       ...
                                       "CONVERT(VARCHAR(4), s.ID_SOLICITUD) else s.NUM_SOLICITUD END),cod2=dbo.RemoveChars(s.NUM_SOLICITUD),s.TIPO_SERVICIO,s.ESTADO_EP,S.ID_TIPO_SOLICITUD,TIPO_CASO=dbo.TIPO_CASO_EPT(S.ID_CASO) FROM SOLICITUDES S " +
                                       ...
                                       " where 1=1 /*s.estado=2*/ and s.id_proyecto=" + context.Request["p"] + reg + sol + com + tipo + ano + mes + rutp + rute + orden + fi + ff + est + di + nc + estep + idprof + idprofev + " ORDER BY " + "S.ID_SOLICITUD" + " " + sortOrderBy);//sortColumnName
            //" /*WHERE S.ESTADO=1*/"
            int total_pages = 0;
                ...
                if (fila >= inicio && fila < (Convert.ToInt32(numberOfRows) * Convert.ToInt32(pageIndex)))
                {
                    xml = xml + "<row id='" + cont.ToString() + "'>" +
                                ...
                                if (dsQuery.Tables[0].Rows[cont]["ID_TIPO_SOLICITUD"].ToString() == "2" && dsQuery.Tables[0].Rows[cont]["TIPO_SERVICIO"].ToString() == "1")
                                {
                                    ...
                                    {
                                        xml = xml + "<cell><![CDATA[<span><a href='#' title='EV.PS. Finalizada' class='ui-icon ui-icon-check' onclick='reabrir(" + dsQuery.Tables[0].Rows[cont]["ID_SOLICITUD"].ToString() + ")'></a></span>]]></cell>";
                                    }
                                    else
                                    {
                                ...
                                else //if (dsQuery.Tables[0].Rows[cont]["ESTADO"].ToString() == "5" || dsQuery.Tables[0].Rows[cont]["ESTADO"].ToString() == "13" || dsQuery.Tables[0].Rows[cont]["ESTADO"].ToString() == "8" || dsQuery.Tables[0].Rows[cont]["ESTADO"].ToString() == "10" || dsQuery.Tables[0].Rows[cont]["ESTADO_EP"].ToString() == "4" || dsQuery.Tables[0].Rows[cont]["ESTADO_EP"].ToString() == "10" || dsQuery.Tables[0].Rows[cont]["ESTADO_EP"].ToString() == "13")
                                {
                    ...
                    xml = xml + "<cell><![CDATA[<span><a href='#' title='Enviar Notificación' class='ui-icon ui-icon-mail-closed' onclick='enviaNotiSol(" + dsQuery.Tables[0].Rows[cont]["ID_SOLICITUD"].ToString() + ")'></a></span>]]></cell>";
                                ...
                                if (dsQuery.Tables[0].Rows[cont]["ESTADO"].ToString() == "7" || dsQuery.Tables[0].Rows[cont]["ESTADO"].ToString() == "9" || dsQuery.Tables[0].Rows[cont]["ESTADO"].ToString() == "0")
                                {
                                    xml = xml + "<cell><![CDATA[<span><a href='#' title='Duplicar' class='ui-icon ui-icon-copy' onclick='duplicar(" + dsQuery.Tables[0].Rows[cont]["ID_SOLICITUD"].ToString() + ")'></a></span>]]></cell>";
                                }
                                else
                                {
                                    xml = xml + "<cell><![CDATA[<span><a href='#' title='No disponible' class='ui-icon ui-icon-cancel'></a></span>]]></cell>";
                                }
                                xml = xml + "<cell><![CDATA[<span><a href='#' title='Envió ECT' class='ui-icon arrow-up-circle-fill' onclick='cambiaEstadoECT(" + dsQuery.Tables[0].Rows[cont]["ID_SOLICITUD"].ToString() + ")'></a></span>]]></cell>";
